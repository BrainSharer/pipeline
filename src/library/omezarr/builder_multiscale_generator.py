...
class BuilderMultiscaleGenerator:
    ...
    def write_transfer(self, client):
        ...
        print(f'Stack after stacking and rechunkunking: type: {type(stack)} shape: {stack.shape} chunks: {stack.chunksize} dtype: {stack.dtype}')
    ...
    def write_rechunk_transfer(self, client, input_path, write_storepath, level):
        ...
        end_time = timer()
        total_elapsed_time = round((end_time - start_time), 2)
        print(f"Rechunking completed in {total_elapsed_time} seconds")
