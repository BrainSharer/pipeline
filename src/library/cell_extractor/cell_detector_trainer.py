...
class DK55DataLoader(CellAnnotationUtilities):
    ...
    def load_refined_original_feature(self):
        dfpath = '/net/birdstore/Active_Atlas_Data/cell_segmentation/DK55/all_features.csv'
        if not os.path.exists(dfpath):
            print(f'file {dfpath} does not exist')
            sys.exit()
        df = pd.read_csv(dfpath, index_col = 0)
        if not os.path.exists(self.QUALIFICATIONS):
            print(f'file {self.QUALIFICATIONS} does not exist')
            sys.exit()
